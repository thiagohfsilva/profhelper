...
class TestQuestionarioGUI(unittest.TestCase):
    def setUp(self):
        ...
        self.questions = {0: 'Nome:', 1: 'Como foi a adaptação?', 2: 'Aspecto emocional?'}
        ...
        self.report_paragraphs = {
            1: {'Boa': ['Adaptação excelente.'], 'Ruim': ['Adaptação difícil.']},
            2: {'Tranquilo': ['Emocional estável.'], 'Ansioso': ['Emocional instável.']}
        }
        ...
        # Patch tkinter root
        self.root = MagicMock()
        self.gui = QuestionarioGUI(self.root, self.questions, self.possible_answers)

    def test_identificar_resposta_por_codigo(self):
        codigo = '01'  # Nome: (ignored), 1: 'Ruim', 2: 'Tranquilo'
        respostas = self.gui.identificar_resposta_por_codigo(self.questions, self.possible_answers, '01')
        self.assertEqual(respostas['Nome:'], self.possible_answers[0][0] if self.possible_answers[0] else 'Sem resposta')
    ...
    def test_gerar_relatorio_paragrafos(self):
        ...
        with patch('random.choice', side_effect=lambda x: x[0]):
            relatorio = self.gui.gerar_relatorio_paragrafos(self.questions, self.possible_answers, respostas)
    ...
    def test_identificar_resposta_por_codigo_invalid(self):
        with self.assertRaises(ValueError):
            self.gui.identificar_resposta_por_codigo(self.questions, self.possible_answers, 'ZZ')
